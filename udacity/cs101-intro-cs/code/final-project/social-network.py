...
def create_data_structure(string_input):
    ...
    Keyword arguments:
    string_input -- a string containing games and friend data, formatted in a specific way
    """
    network = {}
    ...
    for i in range(0, len(sentences)-1, 2):
        ...
        games = map(str.strip, sentences[i+1].split( ' likes to play ')[1].split(','))
        user = connections[0]
...
def get_connections(network, user):
    ...
    Keyword arguments:
    network -- a dictionary containing dictionaries of a user's connections and games
...
def add_connection(network, user_A, user_B):
    ...
    user_B -- the name of a person in the network that will be added to the list of freinds
    """
    try:
...
def get_secondary_connections(network, user):
    ...
    """
    try:
        return list(set([secondary for primary in network[user]['friends'] for secondary in network[primary]['friends']]))
    except KeyError:
        return None
...
def connections_in_common(network, user_A, user_B):
    ...
    """
    try:
        return len([friend for friend in network[user_A]['friends'] if friend in network[user_B]['friends']])
    except KeyError:
        return False
